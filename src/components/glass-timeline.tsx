                ...
                <div className="flex items-center">
                  ...
                  {index < items.length - 1 && (
                    <div
                      className={cn(
                        "w-24 h-0.5 mx-2",
                        item.status === "completed" ? "bg-linear-to-r from-dunn-purple to-dunn-purple-light" : "bg-white/20",
                      )}
                    />
                  )}
                </div>
            ...
            <div className="flex flex-col items-center">
              ...
              {index < items.length - 1 && (
                <div
                  className={cn(
                    "w-0.5 flex-1 mt-2",
                    item.status === "completed" ? "bg-linear-to-b from-dunn-purple to-dunn-purple-light" : "bg-white/20",
                  )}
                />
              )}
            </div>
...
function GlassTimelineNode({ status, icon }: { status?: TimelineItem["status"]; icon?: React.ReactNode }) {
  return (
    <div className="relative">
      {/* Glow for current */}
      {status === "current" && (
        <div className="absolute -inset-2 rounded-full bg-linear-to-r from-dunn-purple/50 to-dunn-purple-light/50 blur-md animate-pulse" />
      )}
      ...
      <div
        className={cn(
          ...
          status === "completed" && "bg-linear-to-br from-dunn-purple to-dunn-purple-light border-dunn-purple-light/50",
          status === "current" && "bg-white/20 backdrop-blur-xl border-dunn-purple/50",
      ...
      >
        {status === "completed" ? (
          ...
          <span className={cn("text-white/80", status === "current" && "text-dunn-purple-light")}>{icon}</span>
        ) : (
          <div className={cn("w-3 h-3 rounded-full", status === "current" ? "bg-dunn-purple-light" : "bg-white/40")} />
        )}
      </div>
    </div>
  )
...
function GlassTimelineCard({ item }: { item: TimelineItem }) {
  ...
  return (
    <div className="relative">
      {isCurrent && (
        <div className="absolute -inset-1 rounded-xl bg-linear-to-r from-dunn-purple/30 to-dunn-purple-light/30 blur-lg opacity-70" />
      )}
